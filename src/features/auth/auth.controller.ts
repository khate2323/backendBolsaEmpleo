import { BadRequestException, Body, Controller, Get, NotFoundException, Post, Req, Request, UnauthorizedException, UseGuards } from '@nestjs/common';
...
export class AuthController {
    ...
    async register(@Body() body: any) {
        if (!body)
            throw new BadRequestException("Todos los campos son requeridos: ['name', 'email', 'password']");

        this.validateFields.validateFieldsMetodo(body, ['name', 'email', 'password']);
        ...
        return await this.dataSource.transaction(async (manager) => {
            ...
            if (newUser) {
                if (roleFound.nombre === RolesEnum.COMPANY) {
                    this.validateFields.validateFieldsMetodo(body, ['nit', 'phone', 'direction']);
                    ...
                    await this.usersService.update(newUser.id, { company: newCompany }, manager);
    ...
    async login(@Body() data: { email: string; password: string }) {
        try {
            if (!data) throw new BadRequestException('Todos los campos son requeridos: [email, password]');
            ...
            if (!user) {
                const roleFound = await this.rolesService.findByRoleName(RolesEnum.EGRERSADO);
                if (roleFound) {
                    const response = await this.egresadosSystemService.getEgresadoByEmailInstitute(email, password);
                    ...
                    if (response) {
                        return await this.dataSource.transaction(async (manager) => {
                            ...
                            const newUser = await this.authService.register(body, manager);
                            const newEgresado = await this.egresadosService.create({ ...response, id_egresado: response.id }, manager)
                            await this.usersService.update(newUser.id, { egresado: newEgresado }, manager);
                            await this.egresadosService.update(newEgresado.id, { user: newUser }, manager)

                            user = newUser

                            // 🟩 Crear token OTP asociado al usuario
                            const tokenData = await this.tokensOtpService.createTokenToUser(user, manager);

                            // 🟩 Enviar email con el token
                            await this.emailService.sendCodeVerificationEmail(user.email, user.name, tokenData.token);
    ...
    async verifyToken(@Body() data: { token: string }, @Req() req: any) {
        try {
            if (!data) throw new BadRequestException("Todos los campos son requeridos: ['token']");
    ...
    async logout(@Req() req: any) {
        try {
            const userId = req.tokenDecoded.payload.sub
            const result = await this.usersService.update(userId, { isSessionActive: false })
            return result
        } catch (error) {
            throw error;
        }
